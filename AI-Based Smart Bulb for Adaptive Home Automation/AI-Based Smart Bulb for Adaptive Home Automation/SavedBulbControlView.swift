...
struct SavedBulbControlView: View {
    ...
    var body: some View {
                ...
                ScrollView {
                    VStack(spacing: 30) {
                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            // Labels: Cool left (value=0), Warm right (value=255)
                            HStack {
                            ...
                            // Gradient: cool white left → warm amber right
                            ZStack(alignment: .center) {
                                RoundedRectangle(cornerRadius: 8)
                                    .fill(
                                        LinearGradient(
                                            colors: [
                                                Color(red: 0.85, green: 0.95, blue: 1.0), // left = cool
                                                Color(red: 1.0,  green: 0.75, blue: 0.4)  // right = warm
                                            ],
                                            startPoint: .leading,
                                            endPoint: .trailing
                                        )
                            ...
                            // Presets: Cool=0 (left), Neutral=128 (middle), Warm=255 (right)
                            HStack(spacing: 12) {
        ...
        .onAppear { connectToBulb() }
        .onDisappear { bleManager.disconnect() }
    }
    
    // MARK: - Connect
    func connectToBulb() {
        if savedBulb.is_simulated {
            guard bleManager.simulatorMode else {
                isConnecting = false; connectionFailed = true; return
            }
