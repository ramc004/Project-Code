...
//
// Presented during the forgot-password flow initiated from LoginView
// Collects the six-digit reset code sent to the user's email and a new password meeting all complexity rules, then submits the reset to the backend

import SwiftUI
import Combine

/// The password reset screen, reached from the forgot-password flow in "LoginView"

/// Receives the user's email address, the six-digit reset code generated in "LoginView", and a binding to "LoginView"'s "loginMessage" so a success message can be displayed after dismissal

/// The user must:
/// 1. Enter the six-digit code before it expires (5-minute countdown)
/// 2. Create a new password satisfying all four complexity rules

/// On a successful reset the view writes a confirmation message to "loginMessage", then dismisses itself back to "LoginView"
/// If the code expires, a Resend button appears to generate and deliver a fresh code, resetting the countdown
struct ResetPasswordView: View {

    // MARK: - Inputs

    /// The email address of the account being reset, passed from "LoginView"
    var email: String

    /// The six-digit reset code generated and sent in "LoginView"
    /// Declared as "@State" so it can be replaced when the user requests a resend
    @State var verificationCode: String

    /// A binding to "LoginView"'s "loginMessage", written on successful reset so the confirmation is visible after this view is dismissed
    ...
    /// Whether the new password meets the minimum length requirement (8+ characters)
    @State private var passwordLengthValid = false

    /// Whether the new password contains at least one uppercase letter
    @State private var passwordUppercaseValid = false

    /// Whether the new password contains at least one numeric digit
    @State private var passwordNumberValid = false

    /// Whether the new password contains at least one special character
    @State private var passwordSpecialCharValid = false

    /// True while the password reset request is in flight
    @State private var resetting = false

    /// An inline error message shown when the reset request fails
    ...
    /// Used to dismiss this view and return to "LoginView" after a successful reset
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading) {
                    ...
                    // Validation fires only when exactly 6 digits have been entered
                    TextField("6-digit code", text: $codeInput)
                        ...
                        .onChange(of: codeInput) { _ in
                            ...
                            // Only compare once the full 6-digit code has been entered
                            if codeInput.count == 6 && !codeExpired {
                                codeValid = codeInput.trimmingCharacters(in: .whitespacesAndNewlines)
                ...
                // Provides real-time feedback as the user types their new password
                ...
                // Inline error message from a failed reset attempt
    ...
    /// Validates the current new password against all four complexity rules using regular expressions, updating the corresponding boolean state properties
    ...
    /// Submits the email and new password to the "/reset_password" endpoint
    
    /// On a HTTP 200 response, writes a success message to the "loginMessage" binding (displayed in "LoginView") and dismisses this view
    ...
    func resetPassword() {
        ...
        request.httpBody = try? JSONSerialization.data(
            withJSONObject: ["email": email, "password": newPassword]
        )
            ...
            DispatchQueue.main.async {
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        // Reset successful, write the confirmation message and return to LoginView
    ...
    /// Generates a new six-digit reset code, sends it to the user's email, and resets the countdown timer to 5 minutes
    ...
    func resendVerificationCode() {
        ...
        request.httpMethod = "POST"
        request.httpBody = try? JSONSerialization.data(
            withJSONObject: ["email": email, "code": verificationCode]
        )
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
