...
// Collects the six-digit reset code sent to the user's email and a new
// password meeting all complexity rules, then submits the reset to the backend.
...
///
/// Receives the user's email address, the six-digit reset code generated in
/// `LoginView`, and a binding to `LoginView`'s `loginMessage` so a success
/// message can be displayed after dismissal.
///
/// The user must:
...
///
/// On a successful reset the view writes a confirmation message to `loginMessage`,
/// then dismisses itself back to `LoginView`. If the code expires, a Resend button
/// appears to generate and deliver a fresh code, resetting the countdown.
struct ResetPasswordView: View {
    ...
    /// A binding to `LoginView`'s `loginMessage`, written on successful reset
    /// so the confirmation is visible after this view is dismissed.
    ...
    /// Validates the current new password against all four complexity rules using
    ...
    ///
    /// On a HTTP 200 response, writes a success message to the `loginMessage`
    /// binding (displayed in `LoginView`) and dismisses this view. Non-200
    ...
    /// Generates a new six-digit reset code, sends it to the user's email,
