...
// Presents the login screen where registered users authenticate with their email and password
// Also handles the forgot-password flow by verifying the email exists, generating a reset code, and navigating to ResetPasswordView

import SwiftUI

/// The login screen for returning users

/// Authenticates the user against the Flask "/login" endpoint
/// On success, the email and login state are persisted to "UserDefaults" and the user is navigated to "HomeView"
...
/// A server-offline banner is shown if the backend cannot be reached, disabling all input and actions until connectivity is restored
struct LoginView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                // Email Field
                // Clears error and success messages on every keystroke to keep the UI state consistent with what the user has typed
                ...
                // Success Message
                // Displayed when navigating back from ResetPasswordView after a successful password reset, prompting the user to log in again
    ...
    /// Clears any stale error message regardless of the result so that the offline banner becomes the sole source of messaging when offline
    ...
    /// On success, the user's email and login flag are persisted to "UserDefaults" and the view navigates to "HomeView"
    /// Failure responses are mapped to user-friendly messages based on the error content returned by the server
    func loginUser() {
        loginMessage = ""
        errorMessage = ""
    ...
    ///
    /// First confirms the email is registered via "/check_email"
    /// If registered, generates a six-digit reset code, sends it via "/send_code", and navigates to "ResetPasswordView"
    /// If the email is not registered, an inline error is shown prompting the user to register instead
